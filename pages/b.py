...
url =st.secrets["url"]
url="https://pi49rvanrsofzq4fnxtng.c0.us-east1.gcp.weaviate.cloud"
...
fs = FeedbackSystem('feedback_loop.txt', embeddings, "https://pi49rvanrsofzq4fnxtng.c0.us-east1.gcp.weaviate.cloud", "nt3RvOGRAbSzaKg6pGfclU36OZqq9dbo6CaC")
...
with open('./feedback_loop.txt', 'r') as f:
  ...
  feedback = f.read()
st.write("i have upserted")
feedback_db.upsert(feedback)
req = RAGEval(vb_list, cross_encoder)
print("this first step 1............ ")
req.model_prep(chat_model, mistral_parser) #, mistral_parser) # model details
q_parser = RunnableLambda(lambda ans: ans.split('\n')[-1].strip()[len('Output: '):])
...
req.feedback_prep(uri='./lancedb/rag', table_name='feedback',
                   embedder=weaviate_embed,
                  splitter=RecursiveCharacterTextSplitter(chunk_size=1330, chunk_overlap=35),file=feedback_file)

req.query_agent_prep(gq_model) #, parser=alt_parser)

# req.feedback_prep('feedback_loop.txt', OpenAIEmbeddings(model='text-embedding-3-large'), 'https://pi49rvanrsofzq4fnxtng.c0.us-east1.gcp.weaviate.cloud', 'nt3RvOGRAbSzaKg6pGfclU36OZqq9dbo6CaC') # file, embedding, url, api):
